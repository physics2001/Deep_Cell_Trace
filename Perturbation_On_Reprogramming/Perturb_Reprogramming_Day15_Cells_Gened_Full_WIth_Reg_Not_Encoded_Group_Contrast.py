# -*- coding: utf-8 -*-
"""
Created on Wed Nov 23 00:19:42 2022
...
gene_num_interesting = [644, 219, 225, 5, 341, 349, 265, 14, 15, 228, 859, 779, 545, 472, 563, 564, 427, 185, 317, 224]

gene_num_not_interesting = [999, 559, 174, 175, 176, 177, 557, 179, 555, 544, 542, 541, 184, 540, 186, 535, 531, 527, 526, 520]
...
for DROP_OUT in drop_out_list: 
    for MODEL_TYPE in model_type_list: 
        for NUM_LAYER in num_layer_list: 
            ...
            RESULT_DF.to_csv("Perturb_Reprogramming_{0}_Genes_Not_Encoded_Day_21_28_Gened_Change_In_Reprog_Result_With_Init_Class_Info_Interesting.csv".format(n_top_genes), index=False)
            ...
            for i in gene_num_not_interesting: 
                X_mod_i[:, :, i] = X_mod_i[:, :, i] * 0.0
            Predicted_Day21 = Reg_Day21_Model.predict(X_mod_i)
            Predicted_Day21 = np.array([Predicted_Day21], dtype=np.float32)
            Predicted_Day21 = np.swapaxes(Predicted_Day21, 0, 1)
            ...
            RESULT_DF = pd.DataFrame(RESULT_LIST)
            RESULT_DF = pd.concat((RESULT_DF, y_Gened_classes), axis=1)
            RESULT_DF.to_csv("Perturb_Reprogramming_{0}_Genes_Not_Encoded_Day_21_28_Gened_Change_In_Reprog_Result_With_Init_Class_Info_Not_Interesting.csv".format(n_top_genes), index=False)
