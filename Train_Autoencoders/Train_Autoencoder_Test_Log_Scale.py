# -*- coding: utf-8 -*-
"""
Created on Tue Nov  1 21:48:15 2022
...
for log in LOG_DATA: 
    for scale in SCALE_DATA: 
        ...
        with h5py.File("{0}_Genes_Logged_{1}_Scaled_{2}_Data.h5".format(n_top_genes, log, scale), 'r') as f:
        ...
        # model_hyperparameters_list = [[4000, 2000, 1000, 0.3, 0.5]]
        for model_hyperparameters in model_hyperparameters_list: 
            ...
            column_names = np.loadtxt("{0}_Genes_Logged_{1}_Scaled_{2}_Column_Names.txt".format(n_top_genes, log, scale), dtype=str)
...
ALL_DF = pd.read_csv('Autoencoder_Result_With_Different_Preprocessing.csv', index_col=0)
ALL_DF = pd.concat([ALL_DF, RESULT_DF], ignore_index=True)
ALL_DF.to_csv("Autoencoder_Result_With_Different_Preprocessing.csv")
