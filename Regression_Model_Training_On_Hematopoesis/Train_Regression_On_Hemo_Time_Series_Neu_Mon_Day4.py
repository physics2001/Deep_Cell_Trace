# -*- coding: utf-8 -*-
"""
Created on Sat Feb  4 14:20:45 2023
...
for n_top_genes in n_top_genes_list: 
    ...
    X_train[:, 1, : ] = np.full([X_train.shape[0], FEATURES], 0)
    
    y_val = X_val[:, 1, : ].copy()
    X_val[:, 1, : ] = np.full([X_val.shape[0], FEATURES], 0)
    
    y_test = X_test[:, 1, : ].copy()
    X_test[:, 1, : ] = np.full([X_test.shape[0], FEATURES], 0)
    ...
    for DROP_OUT in drop_out_list: 
        for MODEL_TYPE in model_type_list: 
            for NUM_LAYER in num_layer_list: 
                reg_folder = "Regression_On_Not_Encoded_Data_Neu_Mon_Two_Classes_Day4/{4}_Genes_Two_Classes_Day6/Features_{0}_Dropout_{1}_NumLayer{2}_{3}/".format(FEATURES, DROP_OUT, NUM_LAYER, MODEL_TYPE, n_top_genes)
                filepath = "model"
                ...
                with open('mse.txt', 'w') as FOB:
                    with redirect_stdout(FOB):
                        print(np.mean(mean_squared_error(y_test, y_test_hat)))
...
RESULT_DF.to_csv("Regression_On_Not_Encoded_Data_Neu_Mon_Two_Classes_Day4.csv")
