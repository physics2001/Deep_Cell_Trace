# -*- coding: utf-8 -*-
"""
Created on Mon Nov 21 20:35:39 2022
...
y_train = X_train[:, 4, : ].copy()
X_train = X_train[:, 0:4, : ]

y_val = X_val[:, 4, : ].copy()
X_val = X_val[:, 0:4, : ]

y_test = X_test[:, 4, : ].copy()
X_test = X_test[:, 0:4, : ]
...
for DROP_OUT in drop_out_list: 
    for MODEL_TYPE in model_type_list: 
        for NUM_LAYER in num_layer_list: 
            reg_folder = "Regression_On_Reprogramming_Data/{4}_Genes_Two_Classes_Day21/Features_{0}_Dropout_{1}_NumLayer{2}_{3}/".format(FEATURES, DROP_OUT, NUM_LAYER, MODEL_TYPE, n_top_genes)
            filepath = "model"
...
RESULT_DF.to_csv("Regression_On_Reprogramming_Day21_{0}_Genes_Two_Classes_Result.csv".format(n_top_genes))
