# -*- coding: utf-8 -*-
"""
Created on Thu Nov  3 22:04:22 2022
    ...
    for log in LOG_DATA: 
        for scale in SCALE_DATA: 
            ...
            for i in range(5864): 
                ...
                clones_day6 = clones.loc[clones['time_info']=='6']
                for i2, row2 in clones_day2.iterrows(): 
                    for i4, row4 in clones_day4.iterrows():
                        for i6, row6 in clones_day6.iterrows(): 
                            # time_series_indices_list.append([i2, i4, i6, row6['clone'], row6['state_info']])
            ...
            # time_series_indices = pd.DataFrame(time_series_indices_list, columns=["Day2", "Day4", "Day6", "clone", "state_info"])
            # time_series_indices.nunique()
            # time_series_indices.to_csv("1000_Encoded_Genes_Time_Series_with_Cell_Indices.csv")
            
            
            # np.random.seed(100)
            # sample_data = training_data_X[np.random.randint(training_data_X.shape[0], size=1000), :, :]
            
            # with h5py.File('1000_Encoded_Genes_Scaled_Time_Series.h5', 'a') as FOB: 
            #     FOB.create_dataset("1000_Encoded_Genes_Scaled_Time_Series", data=sample_data, dtype='f')
            
            with h5py.File(folder+'{0}_Genes_Data_Encoded_Time_Series_With_Class.h5'.format(n_top_genes), 'a') as FOB: 
