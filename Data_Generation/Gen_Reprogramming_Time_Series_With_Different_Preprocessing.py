# -*- coding: utf-8 -*-
"""
Created on Mon Nov 14 21:48:08 2022
...
for log in LOG_DATA: 
    for scale in SCALE_DATA: 
        with h5py.File("Reprogramming_{0}_Genes_Logged_{1}_Scaled_{2}_Data.h5".format(n_top_genes, log, scale), 'a') as FOB: 
            print("Keys: %s" % FOB.keys())
            X = list(FOB["Reprogramming_{0}_Genes_Logged_{1}_Scaled_{2}_Data".format(n_top_genes, log, scale)])
            FOB.close()
        ...
        train_inds_df = pd.read_csv('Reprogramming_Train_Inds_Two_Classes.csv', index_col=0)
        val_inds_df = pd.read_csv('Reprogramming_Val_Inds_Two_Classes.csv', index_col=0)
        test_inds_df = pd.read_csv('Reprogramming_Test_Inds_Two_Classes.csv', index_col=0)
        ...
        with h5py.File('{0}_Genes_Reprogramming_Two_Classes_Data_Time_Series_With_Class.h5'.format(n_top_genes), 'a') as FOB: 
