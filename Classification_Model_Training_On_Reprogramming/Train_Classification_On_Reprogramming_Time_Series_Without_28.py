# -*- coding: utf-8 -*-
"""
Created on Mon Nov 14 23:53:05 2022
...
with h5py.File('{0}_Genes_Reprogramming_Data_Time_Series_With_Class.h5'.format(n_top_genes), 'a') as f: 
    print("Keys: %s" % f.keys())
...
for DROP_OUT in drop_out_list: 
    for MODEL_TYPE in model_type_list: 
        for NUM_LAYER in num_layer_list: 
            folder = "Classification_On_Encoded_Reprogramming_Data/{4}_Genes_Not_Encoded_Without_28/Features_{0}_Dropout_{1}_NumLayer_{2}_ModelType_{3}/".format(FEATURES, DROP_OUT, NUM_LAYER, MODEL_TYPE, n_top_genes)
            filepath = "model"
            os.makedirs(folder)
...
RESULT_DF.to_csv("Classification_Reprogramming_{0}_Genes_Not_Encoded_Without_28_Result.csv".format(n_top_genes))
