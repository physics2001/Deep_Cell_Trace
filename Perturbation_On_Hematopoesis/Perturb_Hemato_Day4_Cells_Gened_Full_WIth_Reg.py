# -*- coding: utf-8 -*-
"""
Created on Sun Nov 20 12:33:41 2022
...
with h5py.File('{0}_Genes_Data_Encoded_Time_Series_Day_6_9_12_15_Two_Classes.h5'.format(n_top_genes), 'a') as FOB: 
    print("Keys: %s" % FOB.keys())
    X = list(FOB['X'])
    FOB.close()
    
with h5py.File('{0}_Genes_Data_Encoded_Time_Series_Gened_Day_21_28_Two_Classes.h5'.format(n_top_genes), 'a') as FOB: 
    print("Keys: %s" % FOB.keys())
    X_Gened = list(FOB['X'])
    FOB.close()
...
Reg_Day21_Model_Path = 'Regression_On_Encoded_Reprogramming_Data/1000_Genes_Two_Classes_Day21/1000_Genes_Logged_True_Scaled_False_Data/Features_75_Dropout_0.3_NumLayer2_LSTM/model'
Reg_Day21_Model = load_model(Reg_Day21_Model_Path)
Reg_Day28_Model_Path = 'Regression_On_Encoded_Reprogramming_Data/1000_Genes_Two_Classes_Day28/1000_Genes_Logged_True_Scaled_False_Data/Features_75_Dropout_0.3_NumLayer2_LSTM/model'
...
RESULT_DF.to_csv("Perturb_Reprogramming_{0}_Genes_Encoded_Day_21_28_Gened_Change_In_Reprog_Result_With_Init_Class_Info.csv".format(n_top_genes), index=False)
